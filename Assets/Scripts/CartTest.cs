...
public class CartTest : MonoBehaviour
{
    ...
    void Start ()
    {
        ...
        else
        {
            ...
            //on the current prefab we have camera object, whit 2 players connected, 2 cameras will spawn as well
            //so we have to delet that one that is not ours, otherwise one player will see camera of the other player
            Destroy(transform.FindChild("Camera").gameObject);
        }
